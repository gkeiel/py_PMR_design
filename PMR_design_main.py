import PMR_design_functions as ctrl
...
def main():
    ...
    A, B, C, D, E = ctrl.ups_model(r, L, c, R)
    Ar, Br = ctrl.pmr_model(f, h, xi)
    Aa, Bq, Ba, Ca, Da, Ea = ctrl.agm_model(A, B, C, E, Ar, Br, h)

    # compute augmented state-feedback gain
    K = ctrl.compute_gain(Aa, Ba, h, s)
    print("K =", K)

    # PMR controller and closed-loop transfer functions
    cl_tf, id_tf = ctrl.get_tf(Aa, Ba, Ca, Da, Ea, Bq, K, h)
