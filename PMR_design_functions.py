...
def ups_model(r: float, L: float, c: float, R: float):
    ...
    D = 0
    E = [[0], [-1/c]]
    return np.array(A), np.array(B), np.array(C), np.array(D), np.array(E)
...
def compute_gain(Aa, Ba, h: int, s: int):
    n = 2*h +2
    lamb_T = -s*np.linspace(1, 10*(h+2), n)
    I = np.eye(n)
    U = np.empty((n, 0))
    W = np.ones(n)
    for i in range(n):
...
def get_tf(Aa, Ba, Ca, Da, Ea, Bq, K, h: int):
    Bk    = Bq*np.vstack((K[1], 0, np.ones((2*h, 1))))
    cl_tf = ss2tf(Aa+Ba*K, Bk, Ca, Da)
    id_tf = ss2tf(Aa+Ba*K, Ea, Ca, Da)
    return cl_tf, id_tf
